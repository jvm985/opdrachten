...
echo "🔧 Configureren van Host Nginx voor Docker Proxy (Poort 9000)..."

# Functie om config te schrijven
create_config() {
    local domain=$1
    local port=$2
...
create_config "docent.irishof.cloud" 9000

# 2. Student Portaal Config
create_config "student.irishof.cloud" 9000
...
if [ $? -eq 0 ]; then
    ...
    echo "🚀 Host Nginx is nu gekoppeld aan Docker poort 9000!"
else
